package com.example.li_ai_code_mother.utils;
...
public class WebScreenshotUtils {
     ...
     * @param compressImage
     */
    private static void compressImage(String originimagePath , String compressImage){
        final float COMPRESSION_QUALITY = 0.3f;
        try {
            ImgUtil.compress(
                    FileUtil.file(originimagePath),
                    FileUtil.file(compressImage),
                    COMPRESSION_QUALITY
            );
        }catch (Exception e){
            log.error("压缩图片失败：{} -> {}",originimagePath,compressImage,e);
    ...
    private static void waitForPageLoad(WebDriver webDriver){
        try {
            ...
            //等待document.readyStatus 为 complete
